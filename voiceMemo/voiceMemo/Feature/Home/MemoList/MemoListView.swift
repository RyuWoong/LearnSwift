            ...
            VStack {
                if !memoListViewModel.memos.isEmpty {
                    CustomNavigationBar(isDisplayLeftBtn: false, rightBtnAction: {
                        memoListViewModel.naivgationRightBtnTapped()
                ...
                if memoListViewModel.memos.isEmpty {
                    ...
                    MemoListContentView().padding(.top,20)
                }
            }
            // 메모 작성 플로팅 아이콘 버튼 뷰
            WriteMemoBtnView().padding(.trailing,20).padding(.bottom,50)
        }
...
private struct AnnouncementView : View {
    fileprivate var body: some View {
        VStack(spacing: 15)  {
            ...
            Image("pencil").renderingMode(.template)
            Text("\"퇴근 9시간전 메모\"")
            Text("\"개발 끝낸 후 퇴근하기!\"")
            Text("\"밀린 알고리즘 공부하기!!\"")
            Spacer()
            ...
            VStack(spacing: 10) {
                HStack {
                    VStack(alignment: .leading) {
                        Text(memo.title).lineLimit(1).font(.system(size: 16)).foregroundColor(.customIconGray)
                    }
                    Spacer()
...
struct MemoListView_Previews: PreviewProvider {
    static var previews: some View {
        MemoListView().environmentObject(PathModel()).environmentObject(MemoListViewModel())
    }
}
